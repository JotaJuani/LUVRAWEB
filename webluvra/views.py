...
def home(request):
    return render(request, 'webluvra/home.html')
...
def product_list(request, category_slug=None):
    ...
    return render(request, 'webluvra/list.html', context)


def product_detail(request, id, slug):
    product = get_object_or_404(Product, id=id, slug=slug, available=True)
    images = product.images.all().order_by('order')  # obtenemos las 3 imágenes ordenadas
    return render(request, 'webluvra/detail.html', {'product': product, 'images': images})

def about(request):
    return render(request, 'webluvra/about.html')
...
def contact_view(request):
    ...
    return render(request, "webluvra/contact.html")

def all_products_random(request):
    products = list(Product.objects.all())
    random.shuffle(products)
    return render(request, 'webluvra/all_products.html', {'products': products})
